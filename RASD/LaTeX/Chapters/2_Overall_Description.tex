...
In this section are presented the State Diagrams of the S\&C system representing all the possibile operation that a User can perform.
\paragraph{Signup.}
When a User wishes to register on S\&C, they are required to input their credentials, including their name, surname,  email and password in a registration form. If the provided credentials are deemed valid (ensuring that the email or nickname is not already in use, and the password meets the criteria of being at least 8 characters long, containing at least one capital letter, a number, and a special character), S\&C will send an email to the User. Upon the User confirming the registration through the provided email link, the new account is successfully created. If the credentials are not correct S\&C shows an error message to the User and redirects him to the signup form page.

Additionally, if the User selects the option "Register as a new Company" during the signup, the system grants additional permissions, such as the ability to publish internship opportunities.
